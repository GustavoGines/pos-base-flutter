                    ...
                    child: SingleChildScrollView(
                      ...
                      child: ConstrainedBox(
                        ...
                        child: DataTable(
                          ...
                          rows: shifts
                              .map((shift) => _buildDataRow(shift, context))
                              .toList(),
                        ),
                      ),
                    ),
  ...
  DataRow _buildDataRow(CashRegisterShift shift, BuildContext context) {
    ...
    return DataRow(
      ...
      cells: [
        ...
        DataCell(
            Text(shift.cashRegisterName ?? 'Caja ${shift.cashRegisterId}')),
        DataCell(
            Text(DateFormat('dd/MM HH:mm').format(shift.openedAt.toLocal()))),
        DataCell(Text(shift.closedAt != null
            ? DateFormat('dd/MM HH:mm').format(shift.closedAt!.toLocal())
            : 'En curso...')),
        ...
        DataCell(
          Container(
            ...
            decoration: BoxDecoration(
              color:
                  shift.isOpen ? Colors.green.shade50 : Colors.blueGrey.shade50,
        ...
        title: Row(
          children: [
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color:
                    shift.isOpen ? Colors.green.shade50 : Colors.grey.shade100,
...
class _StockMovementsTabState extends State<_StockMovementsTab> {
  ...
  Future<void> _fetchStockMovements() async {
    ...
    try {
      ...
      final rawBaseUrl =
          authProvider.repository.remoteDataSource.baseUrl.trim();
                      ...
                      rows: _movements.map<DataRow>((mov) {
                        ...
                        return DataRow(cells: [
                          ...
                          DataCell(Row(
                            mainAxisSize: MainAxisSize.min,
                            children: [
                              Icon(isSystem ? Icons.computer : Icons.person,
                                  size: 16,
                                  color: isSystem
                          ...
                          DataCell(Builder(
                            builder: (context) {
                              final formatter =
                                  NumberFormat.decimalPattern('es_AR');
                              final rawQty =
                                  double.tryParse(mov['quantity'].toString()) ??
                                      0;
                              final absQty = rawQty.abs();

                              final product = mov['product'];
                              final isByWeight =
                                  product?['is_sold_by_weight'] == true ||
                                      product?['is_sold_by_weight'] == 1;
  ...
  Widget build(BuildContext context) {
    ...
    if (_sales.isEmpty) {
      return const Padding(
        ...
        child: Text('No hay ventas registradas en este turno.',
