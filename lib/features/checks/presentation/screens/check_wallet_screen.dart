...
class _CheckWalletScreenState extends State<CheckWalletScreen> {
  ...
  void _confirmAction(
      BuildContext context, int checkId, String actionLabel, String status,
      {Color confirmColor = Colors.blue}) {
    showDialog(
      context: context,
      builder: (ctx) => AlertDialog(
        title:
            Text(status == 'rejected' ? 'Rechazar Cheque' : 'Depositar Cheque'),
  ...
  Widget build(BuildContext context) {
    ...
    final inWalletChecks =
        provider.checks.where((c) => c.status == 'in_wallet').toList();
                                          ...
                                          child: ConstrainedBox(
                                            ...
                                            child: DataTable(
                                              ...
                                              rows: filteredChecks.map((c) {
                                                ...
                                                final isReadyToDeposit =
                                                    c.status == 'in_wallet' &&
                                                        c.paymentDate.isBefore(
                                                            DateTime.now().add(
                                                                const Duration(
                                                                    days: 1)));
