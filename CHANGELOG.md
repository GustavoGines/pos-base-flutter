...
- **[CRÍTICO] Actualizador del Servidor bloqueado por UAC:** Reemplazado `Process.run` por `Process.start` (fire & forget) para lanzar el `updater.exe`. El método anterior bloqueaba el hilo de Flutter esperando que PowerShell+UAC terminara, dejando el diálogo de actualización congelado indefinidamente.
